...
const ForgetPassword = () => {
  ...
  const handleSendEmail = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/user/forgot-password`, { email });
  ...
  const handleResetPassword = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/user/reset-password/${token}`, { password, confirmPassword });
      setMessage(res.data.message);
